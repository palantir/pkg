...
func main() {
	...
	err = ioutil.WriteFile(caCertFile, []byte(caCertStr), 0644)
	if err != nil {
		panic(err)
	}
	err = ioutil.WriteFile(caKeyFile, []byte(caKeyStr), 0644)
	if err != nil {
		panic(err)
	}

	err = ioutil.WriteFile(serverCertFile, []byte(serverCertStr), 0644)
	if err != nil {
		panic(err)
	}
	err = ioutil.WriteFile(serverKeyFile, []byte(serverKeyStr), 0644)
	if err != nil {
		panic(err)
	}

	err = ioutil.WriteFile(clientCertFile, []byte(clientCertStr), 0644)
	if err != nil {
		panic(err)
	}
	err = ioutil.WriteFile(clientKeyFile, []byte(clientKeyStr), 0644)
	if err != nil {
		panic(err)
	}

	combinedCerts := fmt.Sprintf("%s%s", caCertStr, serverCertStr)
	err = ioutil.WriteFile(combinedCertsFile, []byte(combinedCerts), 0644)
	if err != nil {
		panic(err)
	}
	serverCertWithKey := fmt.Sprintf("%s%s", serverCertStr, serverKeyStr)
	err = ioutil.WriteFile(certWithKeyFile, []byte(serverCertWithKey), 0644)
	if err != nil {
		panic(err)
	}
}
