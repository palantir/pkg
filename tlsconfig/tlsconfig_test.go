...
var (
	...
	key1File          string
	cert2File         string
	combinedCertsFile string
	certWithKeyFile   string
)

func TestMain(m *testing.M) {
	dir, cleanup, err := testdir.CreateTempDir("", "certs")
	defer cleanup()
	if err != nil {
		panic(err)
	}
	...
	cert1File = path.Join(dir, "cert-1.pem")
	err = ioutil.WriteFile(cert1File, []byte(cert1), 0644)
	if err != nil {
		panic(err)
	}
	key1File = path.Join(dir, "key-1.pem")
	err = ioutil.WriteFile(key1File, []byte(key1), 0644)
	if err != nil {
		panic(err)
	}

	cert2, _ := newTestKeyPair(1, "test", "test.com")
	cert2File = path.Join(dir, "cert-2.pem")
	err = ioutil.WriteFile(cert2File, []byte(cert2), 0644)
	if err != nil {
		panic(err)
	}

	combinedCerts := fmt.Sprintf("%s\n%s", cert1, cert2)
	combinedCertsFile = path.Join(dir, "combined-certs.pem")
	err = ioutil.WriteFile(combinedCertsFile, []byte(combinedCerts), 0644)
	if err != nil {
		panic(err)
	}

	certWithKey := fmt.Sprintf("%s\n%s", cert1, key1)
	certWithKeyFile = path.Join(dir, "cert-with-key.pem")
