...
func Apply(originalBytes []byte, patch Patch) ([]byte, error) {
	return ApplyUsingYAMLLibrary(NewGoyamlYAMLLibrary(), originalBytes, patch)
}
...
func ApplyUsingYAMLLibrary[NodeT any](yamllib YAMLLibrary[NodeT], originalBytes []byte, patch Patch) ([]byte, error) {
...
func getParentAndLeaf[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path) (YAMLContainer[NodeT], NodeT, error) {
	var (
		currNode      = node
...
func patchAdd[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path, value interface{}, comment string) error {
...
func patchRemove[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path) error {
	parent, _, err := getParentAndLeaf(yamllib, node, path)
	if err != nil {
		return err
	}
...
func patchReplace[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path, value interface{}, comment string) error {
	parent, _, err := getParentAndLeaf(yamllib, node, path)
	if err != nil {
		return err
	}
...
func patchMove[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path, from Path) error {
...
func patchCopy[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path, from Path) error {
...
func patchTest[NodeT any](yamllib YAMLLibrary[NodeT], node NodeT, path Path, testValue interface{}) error {
