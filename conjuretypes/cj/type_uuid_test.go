...
func TestUUID(t *testing.T) {
	tests := []struct {
		...
		{
			Name: "map",
			Test: typeTestCase[map[uuid.UUID]string]{Codec: cj.Map[map[uuid.UUID]string](cj.UUID[uuid.UUID](), cj.String[string]()), Value: map[uuid.UUID]string{
		...
		{
			Name: "invalid",
			Test: typeTestCase[uuid.UUID]{Codec: cj.UUID[uuid.UUID](), JSON: "\"0000\"", SkipTestMarshal: true, ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON string into Go uuid.UUID after offset 6: invalid UUID length: 4"},
		},
	}
