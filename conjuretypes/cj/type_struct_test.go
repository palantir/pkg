...
// TestStructEqual verifies the codec delegates to the type's own Equal method.
// ptrStruct.Equal compares by pointee, so distinct pointers to equal values report
// equal -- the opposite of Go's == on the struct.
...
// ptrStruct.Equal compares by pointee. The Marshal/Unmarshal methods are no-ops,
// present only to satisfy the Struct constructor's constraint.
...
func (s *simpleStruct) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	...
	if kind := tok.Kind(); kind != jsontext.KindBeginObject {
		return cj.NewKindError[simpleStruct](dec, kind, "json object")
	}
	for {
		...
		case jsontext.KindEndObject:
			return nil
		default:
			return cj.NewKindError[simpleStruct](dec, kind, "next key or closing brace for simpleStruct")
		}
	}
}
