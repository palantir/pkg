...
func TestSafeLongValues(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Test: typeTestCase[int64]{Codec: cj.SafeLong[int64](), JSON: `"42"`, SkipTestMarshal: true, ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON string into Go int64 after offset 4: want json int"},
		},
		{
			Name: "too_large_unmarshal",
			Test: typeTestCase[int64]{Codec: cj.SafeLong[int64](), JSON: "9007199254740992", SkipTestMarshal: true,
				ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON number into Go int64 after offset 16: 9007199254740992 is not a valid value for a SafeLong as it is not safely representable in Javascript: must be between -9007199254740991 and 9007199254740991",
			},
		},
		{
			Name: "too_small_unmarshal",
			Test: typeTestCase[int64]{Codec: cj.SafeLong[int64](), JSON: "-9007199254740992", SkipTestMarshal: true,
				ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON number into Go int64 after offset 17: -9007199254740992 is not a valid value for a SafeLong as it is not safely representable in Javascript: must be between -9007199254740991 and 9007199254740991",
			},
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			Test: typeTestCase[map[SL]int]{Codec: cj.Map[map[SL]int](cj.SafeLongMapKey[SL](), cj.Int32[int]()), JSON: `{"9007199254740992":1}`, SkipTestMarshal: true,
				// GoType is the key's SafeLong, not the enclosing map: the key codec stamps its own type at the error site.
				ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON string into Go safelong.SafeLong within \"/9007199254740992\": 9007199254740992 is not a valid value for a SafeLong as it is not safely representable in Javascript: must be between -9007199254740991 and 9007199254740991",
			},
		},
	}
	for _, tc := range tests {
