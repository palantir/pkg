...
type typeTestCase[T any] struct {
	...
	ErrMarshalJSONTo     string // if nonempty, expect MarshalJSONTo to fail with this message
	SkipTestUnmarshal    bool
	ErrUnmarshalJSONFrom string // if nonempty, expect UnmarshalJSONFrom to fail with this message
}
...
func (tc typeTestCase[T]) TestMarshal(t *testing.T) {
	...
	if assert.JSONEqf(t, tc.JSON, got, "unexpected JSON from (%T)(%v).MarshalJSON", tc.Value, tc.Value) {
		// Equivalent values still must match byte-for-byte on formatting/spacing.
		assert.EqualValuesf(t, tc.JSON, got, "unexpected JSON formatting/spacing from (%T)(%v).MarshalJSON", tc.Value, tc.Value)
	}
}

// codecUnmarshaler adapts a Codec[T] to json.UnmarshalerFrom so the harness sees
// the same SemanticError envelope as production: json/v2 fills the "unmarshal"
// action and this records the Go type a bare codec call leaves unset (cf.
// fillGoType in codec.go).
...
func (t *testStruct) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	...
	if kind := tok.Kind(); kind != jsontext.KindBeginObject {
		return cj.NewKindError[testStruct](dec, kind, "testStruct opening brace")
	}
	var seenName bool
	var seenAge bool
	...
	for {
		...
		if kind != jsontext.KindString {
			return cj.NewKindError[testStruct](dec, kind, "ConjureDefinition closing brace or next key")
		}
		switch key.String() {
		case "name":
			if seenName {
				return cj.NewDuplicateFieldKeyError[testStruct](dec)
			}
	...
	if len(missingFields) > 0 {
		return cj.NewMissingFieldsError[testStruct](dec, missingFields)
	}
	if len(unknownMembers) > 0 {
		if strict, _ := json.GetOption(dec.Options(), json.RejectUnknownMembers); strict {
			return cj.NewUnknownFieldsError[testStruct](dec, unknownMembers)
		}
	}
	return nil
}

// requireErrorMatches asserts err matches want, tolerating json/v2's modal verb,
// which randomly alternates per-process between "json: cannot " and "json: unable
// to " to discourage depending on exact wording. Since that prefix is the only
// varying part, we match against the stable suffix.
