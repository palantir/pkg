...
func (anyCodec[T]) UnmarshalJSONFrom(dec *jsontext.Decoder, receiver *T) error {
	if dec.PeekKind() == jsontext.KindNull {
		// Conjure null-coercion (wire spec 5.6.1) forbids coercing a top-level
		// null to a non-optional any; model a nullable any as optional<any>.
		tok, err := dec.ReadToken()
		if err != nil {
			return WrapSyntaxError(dec, err)
		}
		return NewKindError[T](dec, tok.Kind(), "non-optional value")
	}
