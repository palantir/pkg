...
func TestBinary(t *testing.T) {
	tests := []struct {
		...
		{
			Name: "invalid base64",
			Test: typeTestCase[[]byte]{Codec: cj.Binary[[]byte](), JSON: "\"not_base64!@#\"", SkipTestMarshal: true, ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON string into Go []uint8 after offset 15: illegal base64 data at input byte 3"},
		},
		{
			Name: "not a string",
			Test: typeTestCase[[]byte]{Codec: cj.Binary[[]byte](), JSON: "123", SkipTestMarshal: true, ErrUnmarshalJSONFrom: "json: cannot unmarshal JSON number into Go []uint8 after offset 3: want json string"},
		},
		{
			Name: "map",
